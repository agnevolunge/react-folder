...
const CounterPage = () => {
  ...
  const [grades, setGrades] = useState([10, 5, 6, 4])

  const counterButtonsHandler = (numDifference) => setNum(prevNum => prevNum + numDifference)
  const resetHandler = () => setNum(initialNum)
  const inputHandler = event => setNum(event.target.valueAsNumber)
// const inputHandler = event => setNum(Number(event.target.value))
